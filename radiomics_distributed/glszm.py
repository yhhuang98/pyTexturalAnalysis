...
class RadiomicsGLSZM(base.RadiomicsFeaturesBase):
  ...
  def _calculateCoefficients(self,voxelCoordinates=None):
    P_glszm = self._calculateMatrix(voxelCoordinates)
    #self.logger.debug('Calculating GLSZM coefficients')
    

    ps = numpy.sum(P_glszm, 1)  # shape (Nvox, Ns)
    pg = numpy.sum(P_glszm, 2)  # shape (Nvox, Ng)

    ivector = self.coefficients['grayLevels'].astype(float)  # shape (Ng,)
    jvector = numpy.arange(1, P_glszm.shape[2] + 1, dtype=numpy.float64)  # shape (Ns,)

    # Get the number of zones in this GLSZM
    Nz = numpy.sum(P_glszm, (1, 2))  # shape (Nvox,)
  ...
  @staticmethod
  def getSmallAreaEmphasisFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getLargeAreaEmphasisFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getSizeZoneNonUniformityFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getSizeZoneNonUniformityNormalizedFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getZonePercentageFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getZoneVarianceFeatureValue(coefficients):
    r"""
    **9. Zone Variance (ZV)**
    ...
    jvector = coefficients['jvector']
    Nz = coefficients['Nz']
    ps = coefficients['ps'] / Nz[:, None]  # divide by Nz to get the normalized matrix
  ...
  @staticmethod
  def getZoneEntropyFeatureValue(coefficients):
    r"""
    **10. Zone Entropy (ZE)**
    ...
    eps = numpy.spacing(1)
    Nz = coefficients['Nz']
    P_glszm = coefficients['P_glszm']
    p_glszm = P_glszm / Nz[:, None, None]  # divide by Nz to get the normalized matrix

    ze = -numpy.sum(p_glszm * numpy.log2(p_glszm + eps), (1, 2))
    return ze

  @staticmethod
  def getLowGrayLevelZoneEmphasisFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getHighGrayLevelZoneEmphasisFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getSmallAreaLowGrayLevelEmphasisFeatureValue(coefficients):
    r"""
    **13. Small Area Low Gray Level Emphasis (SALGLE)**
    ...
    Nz = coefficients['Nz']
    P_glszm = coefficients['P_glszm']
    lisae = numpy.sum(P_glszm / ((ivector[None, :, None] ** 2) * (jvector[None, None, :] ** 2)), (1, 2)) / Nz
    return lisae

  @staticmethod
  def getSmallAreaHighGrayLevelEmphasisFeatureValue(coefficients):
    r"""
    **14. Small Area High Gray Level Emphasis (SAHGLE)**
    ...
    Nz = coefficients['Nz']
    P_glszm = coefficients['P_glszm']
    hisae = numpy.sum(P_glszm * (ivector[None, :, None] ** 2) / (jvector[None, None, :] ** 2), (1, 2)) / Nz
    return hisae

  @staticmethod
  def getLargeAreaLowGrayLevelEmphasisFeatureValue(coefficients):
    r"""
    **15. Large Area Low Gray Level Emphasis (LALGLE)**
    ...
    Nz = coefficients['Nz']
    P_glszm = coefficients['P_glszm']
    lilae = numpy.sum(P_glszm * (jvector[None, None, :] ** 2) / (ivector[None, :, None] ** 2), (1, 2)) / Nz
    return lilae

  @staticmethod
  def getLargeAreaHighGrayLevelEmphasisFeatureValue(coefficients):
    r"""
    **16. Large Area High Gray Level Emphasis (LAHGLE)**
    ...
    Nz = coefficients['Nz']
    P_glszm = coefficients['P_glszm']
    hilae = numpy.sum(P_glszm * (ivector[None, :, None] ** 2) * (jvector[None, None, :] ** 2), (1, 2)) / Nz
    return hilae
