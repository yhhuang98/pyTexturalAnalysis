...
# def findImageType(imageTypeName):
#   global logger
#   if 'original' in imageTypeName:
#     return {'Original': {}}
#   elif 'log-sigma' in imageTypeName:
#     try:
#       sigma = float(imageTypeName[10:-6].replace('-','.'))
#       return {'LOG': {'sigma': [sigma]}}
#     except Exception as e:
#       logger.error('Cannot parse parameter sigma for LOG image from image type name {0}: {1}'.format(imageTypeName, e))
#   elif 'wavelet' in imageTypeName:
#     parameters = {}
#     try:
#       imageTypeParameters = imageTypeName[7:].split('_')
#       parameters['decompositionNames'] = [imageTypeParameters[1]]
#       if imageTypeParameters[0] != '':
#         try:
#           dix = int(imageTypeParameters[0])
#           parameters['level'] = 1
#           parameters['start_level'] = dix
#         except Exception:
#           logger.error('Cannot parse the wavelet level {0}.'.format(imageTypeParameters[0]))
#     except Exception as e:
#       logger.error('Cannot parse parameters for wavelet image from image type name {0}: {1}'.format(imageTypeName, e))
