...
class RadiomicsNGTDM(base.RadiomicsFeaturesBase):
  ...
  def _calculateCoefficients(self, voxelCoordinates=None):
    ...
    P_ngtdm = self._calculateMatrix(voxelCoordinates)
    Nvp = numpy.sum(P_ngtdm[:, :, 0], 1)  # shape (Nvox,)
    coefficients = dict()
    coefficients['P_ngtdm'] = P_ngtdm
    coefficients['Nvp'] = Nvp  # shape (Nv,)

    # Normalize P_ngtdm[:, 0] (= n_i) to obtain p_i
    coefficients['p_i'] = P_ngtdm[:, :, 0] / Nvp[:, None]

    coefficients['s_i'] = P_ngtdm[:, :, 1]
    coefficients['ivector'] = P_ngtdm[:, :, 2]

    # Ngp = number of graylevels, for which p_i > 0
    coefficients['Ngp'] = numpy.sum(P_ngtdm[:, :, 0] > 0, 1)

    p_zero = numpy.where(coefficients['p_i'] == 0)
    coefficients['p_zero'] = p_zero
      
    return coefficients

  @staticmethod
  def getCoarsenessFeatureValue(coefficients):
    r"""
  ...
  def getContrastFeatureValue(coefficients):
    ...
    """
    Ngp = coefficients['Ngp']  # shape (Nvox,)
    Nvp = coefficients['Nvp']  # shape (Nvox,)
    p_i = coefficients['p_i']  # shape (Nvox, Ng)
    s_i = coefficients['s_i']  # shape (Nvox, Ng)
    i = coefficients['ivector']  # shape (Ng,)
  ...
  @staticmethod
  def getBusynessFeatureValue(coefficients):
    r"""
    ...
    """
    p_i = coefficients['p_i']  # shape (Nv, Ngp)
    s_i = coefficients['s_i']  # shape (Nv, Ngp)
    i = coefficients['ivector']  # shape (Nv, Ngp)
    p_zero = coefficients['p_zero']  # shape (2, z)
  ...
  @staticmethod
  def getComplexityFeatureValue(coefficients):
    r"""
  ...
  @staticmethod
  def getStrengthFeatureValue(coefficients):
    r"""
